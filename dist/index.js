...
var OK_REGEXP = /\:ok$/i;
/**操作失败判断正则 */
var FAIL_REGEXP = /\:fail\s+/;
/**检测接口返回数据是否成功 */
function check(msg) {
    return OK_REGEXP.test(String(msg));
}
/**默认错误信息 */
var DEF_FAIL_MSG = "未知错误";
/**
 * 获取默认错误信息
 * @param msg 微信返回的信息
...
function getErrMsg(msg, def) {
    ...
    var theDefMsg = def === false ? msg : isString(def) ? String(def) : DEF_FAIL_MSG;
    if (msg && msg.split) {
        var reMsg = msg.split(FAIL_REGEXP)[1];
        return reMsg ? reMsg : theDefMsg;
    }
    return theDefMsg;
}

exports.FAIL_REGEXP = FAIL_REGEXP;
